    ...
    stage('Publish') {
      ...
      steps {
        withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: awsCredentialsId, accessKeyVariable: 'AWS_ACCESS_KEY_ID', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
          sh "aws s3 sync build/site/ s3://${s3Bucket}/ --exclude='_font/*' --recursive --only-show-errors --acl=public-read --cache-control=public,max-age=0,must-revalidate"
          sh "aws s3 sync build/site/ s3://${s3Bucket}/ --include='_font/*' --recursive --only-show-errors --acl=public-read --cache-control=public,max-age=604800 --content-type=application/font-woff"
